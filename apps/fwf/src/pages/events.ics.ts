...
import { siteId, forum } from "../content";
...
export const GET: APIRoute = () => {
  ...
  // The forum is this site rather than an entry on it, so its page is the
  // home page — /events/<slug>/ was never built for it. It still belongs in
  // the calendar: it is the date someone subscribes to fwf.lol for.
  const body = toIcsCalendar(
    items,
    (item) => (item.slug === forum.slug ? `${SITE}/` : `${SITE}/events/${item.slug}/`),
    {
      name: "Future World Forum — events",
      description: "The forum's programme and the events around it, with dates as the organiser publishes them.",
      url: `${SITE}/events/`,
