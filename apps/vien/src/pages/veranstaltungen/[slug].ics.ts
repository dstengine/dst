// Serves /veranstaltungen/<slug>.ics so "add to calendar" is a plain file download,
...
export function getStaticPaths() {
  const items = eventsBySite("vien").filter((i) => Array.isArray(i.body) && i.body.length > 0);
  return items.map((item) => ({ params: { slug: item.slug }, props: { item } }));
}

export const GET: APIRoute = ({ props }) => {
  const { item } = props as { item: Parameters<typeof toIcs>[0] };
  return new Response(toIcs(item, `${SITE}/veranstaltungen/${item.slug}/`), {
    headers: {
