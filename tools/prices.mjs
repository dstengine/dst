...
function fromText(html) {
  const text = html
    ...
    .replace(/\s+/g, " ");
  const out = [];
  for (const m of text.matchAll(/from\s*([£€$¥])\s?(\d{1,4}(?:[.,]\d{2})?)/gi)) {
...
async function probe(url, wanted) {
  try {
    ...
    const ld = fromJsonLd(html, wanted);
    const txt = fromText(html);
...
for (const [key, url] of rows) {
  const r = await probe(url, SHOW_NAME[key.split("/")[0]]);
  let what;
  if (r.note) {
    what = `— ${r.note}`;
  } else if (r.ld?.length) {
